...
public:
  ...
  // computes a 'fitness' distribution from 'penalty' scores
  // we define 'fitness' as 'max_penalty' - 'penalty'
  template <typename InIt>
  void mutate_from_penalty(InIt first, InIt last)
  {
    const auto it = std::max_element(first, last);
    if (it != last) {
      mutate_from_penalty(first, last, *it);
    }
  }
    
  // computes a 'fitness' distribution from 'penalty' scores
  // we define 'fitness' as max_penalty - 'penalty'
  template <typename InIt, typename U>
  void mutate_from_penalty(InIt first, InIt last, U max_penalty)
  {
    const auto mp = static_cast<typename std::iterator_traits<InIt>::value_type>(max_penalty);
    mutate_transform(first, last, [=](auto p) { return mp - p; });
  }
